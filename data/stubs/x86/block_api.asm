...
_blkapi{{ ___blkapi_nsid }}_next_mod:                ;
  ...
  movzx ecx, word ptr [edx+0x26] ; Set ECX to the length we want to check
...
_blkapi{{ ___blkapi_nsid }}_not_lowercase:           ;
  ror edi, 0x0d          ; Rotate right our hash value
...
_blkapi{{ ___blkapi_nsid }}_get_next_func:           ;
  ...
  mov esi, [ebx+ecx*0x04]   ; Get rva of next module name
...
_blkapi{{ ___blkapi_nsid }}_loop_funcname:           ;
  ...
  add edi, [ebp-0x08]       ; Add the current module hash to the function hash
  ...
  add ebx, edx              ; Add the modules base address
  mov cx, [ebx+0x02*ecx]    ; Get the desired functions ordinal
  mov ebx, [eax+0x1c]       ; Get the function addresses table rva
  add ebx, edx              ; Add the modules base address
  mov eax, [ebx+0x04*ecx]   ; Get the desired functions RVA
